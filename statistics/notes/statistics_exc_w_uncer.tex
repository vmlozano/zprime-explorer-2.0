...
\end{flushright}
 ...
 \vspace{1cm}
\begin{center}
  {\color{desycyan}{\bf {\huge  \textsf{Notes on statistics: \\ Discovery and exclusion}}}\color{desyorange}{\bf {\huge  \textsf{.}}}}
\end{center}
...
\begin{align}
 q_0 = \left \{ 
 \begin{matrix} 
 -2\ln \lambda (0) & \mbox{if }\quad\hat{\mu}\geq 0,\\ 0 & \mbox{if }\quad\hat{\mu}< 0\end{matrix}\right.  
\end{align}

\begin{align}
q_\mu = \left \{ 
\begin{matrix} 
-2\ln \lambda (\mu) & \mbox{if }\quad\hat{\mu}\geq \mu,\\ 0 & \mbox{if }\quad\hat{\mu}< \mu\end{matrix}\right.  
\end{align}

where $\lambda(0)= L(0)/L(\hat\mu)$ and $\lambda(\mu)=L(\mu)/L(\hat\mu)$
...
\begin{align}
{\rm med}[Z_0|\mu']=\sqrt{q_{0,A}}
\end{align}

\begin{align}
{\rm med}[Z_\mu|0]=\sqrt{q_{\mu,A}}
\end{align}
...
If we know the number of background events, $b$, and we parameterise the number of signal events as $\mu s$, the likelihood can be written as
...
\end{align}
given the condition that maximises the likelihood, $\hat{\mu}=(n-b)/s$, we have $\hat{\mu}s+b=n$ so 
\begin{align}
L(\hat{\mu})=\frac{n^n}{n!}e^{-n}.
\label{eq:muhat}
\end{align}
Then in order to compute the likelihood,
\begin{align}
...
\end{align}
If we evaluate the Asimov set for the nominal value $\mu'=1$ that according to Eq.~\eqref{eq:testasimov} basically means $n=s+b$,
\begin{align}
...
L(\mu) = \frac{(\mu s + b)^n}{n!}e^{-(\mu s + b)}.
\end{align}
and we take $L(\hat{\mu})$ from Eq.~\eqref{eq:muhat} so,
\begin{align}
...
\end{align}
We take the value $\mu'=0$ so according to Eq.~\eqref{eq:testasimov} $n=b$ and renaming $\mu s\to s$, then,
\begin{align}
{\rm med}[Z_\mu|0]=\sqrt{q_{\mu,A}}=\sqrt{2\left[s+b\ln b-b\ln(s+b)\right]}=\sqrt{2\left[b\ln\left(\frac{b}{s+b}\right)+s\right]}
\end{align}
\begin{align}
\boxed{{\rm med}[Z_0|0]=\sqrt{q_{\mu,A}}=\sqrt{2\left[b\ln\left(\frac{b}{s+b}\right)+s\right]}.}
\end{align}
...
\begin{align}
L(0,\doublehat{b})=\frac{(\doublehat{b}(0))^n}{n!}e^{-\doublehat{b}(0)}\frac{(\tau\doublehat{b}(0))^m}{m!}e^{-\tau\doublehat{b}(0)}=\frac{\left(\frac{n+m}{1+\tau}\right)^n}{n!}e^{-\left(\frac{n+m}{1+\tau}\right)}\frac{\left(\tau\left(\frac{n+m}{1+\tau}\right)\right)^m}{m!}
\end{align}
...
=&\left[-2\left((s+b)\ln\left[\frac{(s+b(\sigma_b^2+b))}{(s+b)(\sigma_b^2+b)}\right]+\frac{b^2}{\sigma_b^2}\ln\left[1+\frac{s\sigma_b^2}{b(b+\sigma_b^2)}\right]\right)\right]^{1/2}
\end{align}

\begin{align}
\boxed{{\rm med}[Z_0|1]=\left[
2\left((s+b)\ln\left[\frac{(s+b(\sigma_b^2+b))}{(s+b)(\sigma_b^2+b)}\right]-\frac{b^2}{\sigma_b^2}\ln\left[1+\frac{\sigma_b^2s}{b(b+\sigma_b^2)}\right]\right)
\right]^{1/2}.}
\end{align}
